...
copyshow:
    li $t1,0            #t1=i=0
		li $t2,0            #t2=j=0
		lw $t3,N            #t3=N
		la $t4,board        #load addr of board
		la $t5,newBoard       #load addr of nb
loop7:		
		bge $t1,$t3,end_loop7       #if i>=N break
		j loop8                 #else go to next loop
loop8:
    bge $t2,$t3,end_loop8       #if j>=N break
		la  $t4,board               #reload add every time 
		la  $t5,newBoard          
		mul $t1,$t1,10              #i=i*10
		
		add $t4,$t4,$t1           #t4=b=board[i]
		add $t4,$t4,$t2         #t4=board[i][j]
	  add $t5,$t5,$t1         #t5=newb[i]
		add $t5,$t5,$t2           #t5=newb[i][j]

    lb $s5,($t5)            #load byte of newb[i][j] store in s5
    
		sb $s5,($t4)              #t4=b[i][j]=s5=newb[i][j]
	  lb $a0,($t4)                #load byte of b[i][j]
		beqz $a0,printdot         #if b[i][j]=0 print dot
		j printo                  #else print#
end_loop8:
		la $a0,eol          #end loop8,print new line j=0 i=i+1 to loop7
		li $v0,4
		syscall
